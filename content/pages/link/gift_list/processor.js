...
class Processor {
  ...
  buildHeader() {
    const header =
      '\uFEFF用户id,用户昵称,收礼时间,礼物id,礼物名称,数量,银瓜子,金瓜子,金仓鼠,ios_金瓜子,ios_金仓鼠,normal_金瓜子,normal_金仓鼠\n';
    this.csvAll = header;
  }
  ...
  buildRecord({
    ...
    const record =
      `"${uid}","${Processor.replace(uname)}","${time}","${gift_id}",` +
      `"${Processor.replace(gift_name)}","${gift_num}","${silver}","${
        !silver ? gold : 0
      }","${hamster}",` +
